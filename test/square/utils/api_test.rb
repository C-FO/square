...
describe Square::Utils::API do
  ...
  describe '#parse_next_link_params' do
    let(:response_header_link){
      "Link: <https://connect.squareup.com/v1/me/payments?#{next_link_params_str}>;rel='next'"
    }

    let(:next_link_params_str){
      "begin_time=2014-01-01T14%3A59%3A59%2B00%3A00&end_time=2014-12-30T20%3A59%3A59Z&batch_token=#{batch_token_str}"
    }
    ...
    it 'returns the correct next link params string' do
      @clean_room.send(:parse_next_link_params, response_header_link).must_equal next_link_params_str
    end
  end
end
